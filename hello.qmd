...
Before we begin writing Python code, you need to install Python3 in your computer.
...
2.  I recommend you use the "IDLE" IDE (Integrated Development Environment), which comes with the python installation for Windows and Mac.
3.  If you are unable to install anything in your machine (you use a Chromebook, for example), you can use a cloud-based development environment such as <https://vscode.dev/> but I strongly recommend using a machine you are able to install python into.
...
Ideally, you will be using an IDE (Integrated Development Environment) which facilitates coding by integrating a **text editor**, a **console** and other tools into one or two windows. IDLE, the IDE that comes with your python installation, is the one I recommend for this course.
...
4.  Run the interpreter with the command `python program.py` to run the program. If using IDLE, you just find the Run Module button, and it will run the program in a separate window, with the output of the program printed in that separate window.
