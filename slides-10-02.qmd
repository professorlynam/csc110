...
It reads the file with filename, iterates over each line in the [numbers.txt](numbers.txt) and return the sum.
...
*   Both functions takes a string `file_name` as argument: 
    ...
    *  `write_word_count` calls `count_words`, takes the returned dictionaries and writes the key value pairs, separated by a comma, to a file named "out_" + `file_name`. 
